...
Вы можете использовать `index.routes.js` для запросов, которые не относятся к `Category` или к `Article`. Ваша корневая страница `/`, куда ведет ссылка, хорошо подходит для `index.routes.js`
...
### Release 2: Добавьте один заключительный компонент
